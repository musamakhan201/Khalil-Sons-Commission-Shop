...
namespace KSCS
{
    public partial class farmers : UserControl
    {
        ...
        private void farmers_Load(object sender, EventArgs e)
        {
            ...
            sc.Open();
            SqlCommand s = new SqlCommand("select FARMER_ID from farmer order by FARMER_ID Desc",sc);
            ...
            {
                fid.Items.Add(dr["FARMER_ID"].ToString());
            }
            sc.Close();
        }
        ...
        private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    sc.Open();
                    string selectedid = dataGridView4.CurrentRow.Cells["FARMER_ID"].Value.ToString();
                    string selectedname = dataGridView4.CurrentRow.Cells["FARMER_NAME"].Value.ToString();
                    string selectedphone = dataGridView4.CurrentRow.Cells["FARMER_PHONE#"].Value.ToString();
                    string selectedcnic = dataGridView4.CurrentRow.Cells["FARMER_CNIC"].Value.ToString();
                    string selectedland = dataGridView4.CurrentRow.Cells["FARMER_LAND"].Value.ToString();
                    string selectedcrops = dataGridView4.CurrentRow.Cells["CROPS_SOW"].Value.ToString();
                    ...
                    string delete = "delete from farmer where FARMER_ID='" + selectedid + "' and FARMER_NAME='" + selectedname + "' and FARMER_PHONE#='" + selectedphone + "' and FARMER_CNIC='" + selectedcnic + "' and FARMER_LAND='" + selectedland + "' and CROPS_SOW='" + selectedcrops + "' and ADDRESS='" + selectedADDRESS + "'and DATE='" + selecteddate + "'";
                    SqlCommand cmd = new SqlCommand(delete, sc);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Farmer Deleted Successfully.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    string sq = ("select * from farmer order by FARMER_ID");
        ...
        private void fid_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                sc.Open();
                string sql = ("select * from farmer where FARMER_ID='" + fid.SelectedItem.ToString() + "'");
                SqlDataAdapter sda = new SqlDataAdapter(sql, sc);
                SqlCommand cmd = new SqlCommand(sql, sc);
                cmd.ExecuteNonQuery();
                DataTable dt = new DataTable();
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ename.Text = dr["FARMER_NAME"].ToString();
                    ephone.Text = dr["FARMER_PHONE#"].ToString();
                    ecnic.Text = dr["FARMER_CNIC"].ToString();
                    eland.Text = dr["FARMER_LAND"].ToString();
                    ecrops.Text = dr["CROPS_SOW"].ToString();
        ...
        private void search_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar==(char)13)
            {
                ...
                dv.RowFilter = string.Format("FARMER_NAME like '%{0}%'",search.Text);
        ...
        private void add_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (land.Text.Trim() == string.Empty)
                {
                    MessageBox.Show("Please Enter Land.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    land.LineFocusedColor = System.Drawing.Color.Red;
                    land.Focus();
                }

                else if (crops.Text.Trim() == "Select")
                {
                ...
                else
                {
                    ...
                    sc.Open();
                    string sql = ("insert into farmer values ('" + idno.Text + "','" + name.Text + "', '" + phone.Text + "', '" + cnic.Text + "', '" + land.Text + "', '" + crops.selectedValue + "', '" + address.Text + "','" + date.Text + "')");

                    SqlCommand comd = new SqlCommand(sql, sc);

                    comd.ExecuteNonQuery();

                    MessageBox.Show("Farmer Added Successfully.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    name.Text = "";
                    phone.Text = "";
                    cnic.Text = "";
                    address.Text = "";
                    land.Text = "";
                    crops.selectedIndex = 0;
        ...
        {
            exportexcel("Farmers");
            System.Diagnostics.Process.Start(@"C:\Users\zaeem  hassan\Documents\Farmers.xlsx");
        }

        private void exppdf_Click_1(object sender, EventArgs e)
        {
            exportpdf(dataGridView1, "Farmers");
            System.Diagnostics.Process.Start(@"C:\Users\zaeem  hassan\Documents\Farmers.pdf");
        }
        ...
        private void update_Click_1(object sender, EventArgs e)
        {
            ...
            {
                sc.Open();
                string sql = ("update farmer set FARMER_NAME='" + ename.Text + "', FARMER_PHONE#='" + ephone.Text + "',FARMER_CNIC='" + ecnic.Text + "', FARMER_LAND='" + eland.Text + "', CROPS_SOW='" + ecrops.Text + "', ADDRESS='" + eaddress.Text + "' where FARMER_ID='" + fid.Text + "'");

                SqlCommand comd = new SqlCommand(sql, sc);

                comd.ExecuteNonQuery();

                MessageBox.Show("Farmer Updated Successfully.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ename.Text = "";
                ephone.Text = "";
                ecnic.Text = "";
                eaddress.Text = "";
